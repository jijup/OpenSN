...
void Analysis::runAnalysis(std::vector<Noise::Point> points, int pairingFunction, int noiseType, int width, int height, int ampAnalysis, int fournierAnalysis) {
    ...
    if (ampAnalysis == 1) {
        ...
        std::string title = "../Analysis/AmplitudeAnalysis_Pair" + std::to_string(pairingFunction) + "_Noise" + std::to_string(noiseType)
             + "_W" + std::to_string(width) + "_H" + std::to_string(height);
